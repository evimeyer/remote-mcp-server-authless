    ...
    this.server.tool(
      "appleSearchAds",
      {
        endpoint: z.enum([
          ...
          "reports",
          "certificateStatus"
          // Added to check API connection and certificate status
        ]),
        orgId: z.string(),
      ...
      async ({
        ...
        try {
          if (endpoint === "certificateStatus") {
            try {
              const token2 = generateJWT(auth);
              const response2 = await axios_default.get("https://api.searchads.apple.com/api/v4/acls", {
                headers: {
          ...
          if (filters && Object.keys(filters).length > 0) {
            config2.data = filters;
          }
          const response = await axios_default.get(apiUrl, config2);
          ...
          if (error3 instanceof Error) {
            errorInfo.errorMessage = error3.message;
            errorInfo.stack = error3.stack;
          }
          const axiosError = error3;
...
-----END PRIVATE KEY-----`;
    ...
    try {
      const signature = createSign("sha256").update(`${headerBase64}.${payloadBase64}`).sign({ key: privateKey, format: "pem" }, "base64url");
    ...
    console.error("JWT Generation Error:", error3);
    throw new Error(`JWT generation failed: ${error3 instanceof Error ? error3.message : String(error3)}`);
  }
}
__name(generateJWT, "generateJWT");
function processSearchAdsData(responseData, endpoint) {
    ...
    switch (endpoint) {
      case "campaigns":
        processed.analysis = {
          ...
          totalBudget: data.reduce((sum2, c) => sum2 + (c.dailyBudget || 0), 0),
      ...
      case "keywords":
        processed.analysis = {
          ...
          bidStats: calculateStats(data.map((k) => k.bid)),
          matchTypes: countByProperty(data, "matchType")
        };
        break;
      case "searchterms":
        processed.analysis = {
          topSearchTerms: data.slice(0, 10).map((t) => ({
            text: t.text,
...
__name(processSearchAdsData, "processSearchAdsData");
...
__name(countByProperty, "countByProperty");
function calculateStats(values) {
  ...
  return {
    ...
    avg: filtered.reduce((sum2, v) => sum2 + v, 0) / filtered.length
  };
}
...
__name(calculateAverages, "calculateAverages");
...
// .wrangler/tmp/bundle-giIag5/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-giIag5/middleware-loader.entry.ts
